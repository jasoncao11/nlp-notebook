...
def load_chinese_base_vocab(vocab_path, simplfied=False, startswith=["[PAD]", "[UNK]", "[CLS]", "[SEP]", "[MASK]"]):
    """
    加载官方中文bert模型字典
    simplified: 是否简化词典
    """
...
class BasicTokenizer(object):
    ...
    def encode(self,
        ...
        first_segment_ids_for_mask = [1] * len(first_token_ids)

        if second_text is not None:
            second_token_ids = self.tokens_to_ids(second_tokens)
            second_segment_ids = [1] * len(second_token_ids)
            second_segment_ids_for_mask = [0] * len(second_token_ids)
            first_token_ids.extend(second_token_ids)
            first_segment_ids.extend(second_segment_ids)
            first_segment_ids_for_mask.extend(second_segment_ids_for_mask)
            
        if is_segment:
            return first_token_ids, first_segment_ids, first_segment_ids_for_mask
        else:
...
#     word2idx, keep_tokens = load_chinese_base_vocab(vocab_path="../bert-base-chinese/vocab.txt", simplfied=True)
#     # print(word2idx)
#     # print(keep_tokens)
#     # print(len(word2idx))
...
#     input_ids, segment_ids, segment_ids_for_mask = tokenizer.encode("你好啊，今天过的怎么样？", "我很好，谢谢你啦", max_length=15)
#     print(input_ids)
#     print(segment_ids)
#     print(segment_ids_for_mask)
