...
class TextRCNN_Bert(nn.Module):
    def __init__(self, bert_path, class_num):
        ...
        self.lstm = nn.LSTM(768, 32, bidirectional=True, batch_first=True)
        self.fc = nn.Linear(32 * 2 + 768, class_num)
    ...
    def forward(self, context, mask):
        ...
        out, _ = self.lstm(second_to_last_layer) # [batch_size, seq_len, 32 * 2]
        out = torch.cat((second_to_last_layer, out), 2) # [batch_size, seq_len, 32 * 2 + 768]
        out = F.relu(out) # [batch_size, seq_len, 32 * 2 + 768]
        out = out.permute(0, 2, 1) # [batch_size, 32 * 2 + 768, seq_len]
        out = F.max_pool1d(out, out.size(2)).squeeze(2) # [batch_size, 32 * 2 + 768]        
